...
while read -r num; do
  ...
  shasum=$(sha512sum <<< "$num")
  exec 9>"/tmp/factordb-composites/$shasum"
  if flock -xn 9; then
      declare factor
      start_time="$(date +%s%N)"
      while read -r factor; do (
        echo "Found factor ${factor} of ${num}"
        output=$(sem --id 'factordb-curl' --fg -j 1 xargs curl --retry 10 --retry-all-errors --retry-delay 10 --connect-timeout 60 --max-time 60 \
          --url-query "number=${num}" --url-query "factor=${factor}" <<< "https://factordb.com/reportfactor.php")
        error=$?
        ...
        ./yafu -threads 4 -R -qssave "./qs" -session "./session" -logfile "./log" -o "./nfs" -pscreen -inmem 2000000000 <<<"factor(${num})" 2>&1 \
          | tee "./out" \
          | grep -i '\(P[0-9]\+\|factor\) = ' | grep -o '= [0-9]\+' | grep -o '[0-9]\+' \
          | awk '!x[$0]++'
