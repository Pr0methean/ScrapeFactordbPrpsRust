...
while read -r num; do
  ...
  if flock -xn 9; then
      ...
      while read -r factor; do (
        ...
        output=$(sem --id 'factordb-curl' --fg -j 2 curl -X POST --retry 10 --retry-all-errors --retry-delay 10 https://factordb.com/reportfactor.php -d "number=${num}&factor=${factor}")
        error=$?
